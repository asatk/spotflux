...
double **adi(double **grid, double **newgrid, double *flow, double *grad, double *difr,
        int ntheta, int nphi, double dt) {
...
double **lax(double **grid, double **newgrid, double *flow, double *grad, double *difr,
        int ntheta, int nphi, double dt) {
    ...
    for ( i = 1 ; i < ntheta - 1; i++ ) {
        ...
        for ( j = 0 ; j < nphi ; j++ ) {
            ...
            // new value of field at (theta[i], phi[j])
            // term 0 - Lax method avg
            bij = (bipj + bimj + bijp + bijm) / 4;
            val = bij / dt;
